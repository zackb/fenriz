...
|---|---|---|
| xdg-foreign-v1 / xdg-foreign-v2 (`wlr_xdg_foreign_v1`, `wlr_xdg_foreign_v2`, `wlr_xdg_foreign_registry`) | Cross-process surface parenting: portal and file-chooser dialogs attach to the window that opened them instead of floating free. wlroots tracks the relationship itself, so it is three create calls | S |
| xdg-dialog-v1 (`wlr_xdg_dialog_v1`) | Clients mark modals; a tiler should float and center them rather than tile a 300x150 save dialog. The global alone is a no-op — the value is in routing it to the same path `windowrule ... float` uses | S–M |
