...
| xdg-system-bell-v1 | `wlr_xdg_system_bell_v1` (v1) | `server.cpp`, `view.cpp` | The terminal bell reaches the compositor instead of going nowhere. fenriz has no audio, so a ring flags the window it names as urgent for the bar (`windows[].urgent` in [IPC.md](IPC.md)), which focusing it clears. Shares `mark_urgent` with xdg-activation but not its rule: an activation for a window already on screen is noise, while a bell in a visible-but-unfocused window is the case worth showing. A bell naming no surface is dropped — there is no window to flag |
