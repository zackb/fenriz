...
| xdg-toplevel-icon-v1 | `wlr_xdg_toplevel_icon_manager_v1` (v1) | `server.cpp`, `ipc.cpp` | Per-window icons for bars and window lists, better than guessing from app_id. The XDG icon-theme **name** is kept and published as `windows[].icon` in [IPC.md](IPC.md); pixel-buffer icons are dropped, since the feed is text and neither foreign-toplevel protocol carries an icon. No `icon_size` preferences are advertised — that list only guides clients drawing buffers |
